...
class ScaQr(Agent, RAGMixin):
    """
    ScaQr will keep retrieving documents and rewriting queries until receiving a sufficient context signal from the LLM.
    """
    def __init__(
        self,
        text_corpus: pd.DataFrame,
        llm_prompt: LLMPrompt,
        sca_with_qr_example: str = "",
        sca_max_steps: int = 5,
        ...
        Args:
            text_corpus (pd.DataFrame): The text corpus to search for answers.
                Must contain two columns: 'title' and 'article'.
            llm_prompt (LLMPrompt): The prompt to be used by the agent.
            sca_with_qr_example (str): Prompt examples to include in agent prompt.
                Defaults to "".
        ...
        self.sca_with_qr_example = sca_with_qr_example
    ...
    def _parse_response(self, response_text: str):
        """
        Parses the LLM's response to check for context sufficiency and query rewrite result.

        Args:
            response_text: The text generated by the language model.

        Returns:
            1) True if the context is sufficient, False otherwise.
            2) The query rewrite result.
        """
        ...
        return json_obj.get("Sufficient Context") == 1, json_obj.get("Query Rewrite")
    ...
    def _build_agent_prompt(self, question: str) -> str:
        ...
        self.evidence = "\n================\n\n".join([self.evidence, self.get_RAG_evidence(question)])
        logger.debug(f"\n\t>>> evidence: {self.evidence}\n")
        return self.llm_prompt.get_prompt().format(
            evidence=self.evidence, example=self.sca_with_qr_example, question=question
        )
        ...
        **kwargs,
        ...
        while (self.step_round <= self.sca_max_steps) and (not self.is_sufficient):
            try:
                ...
                is_sufficient, query_rewrite = self._parse_response(response.pred)
                if is_sufficient:
                  self.is_sufficient = True
                self.step_round += 1
                self.agent_interactions.statuses.append(Status(is_sufficient=self.is_sufficient, query_rewrite=query_rewrite))
            except Exception:
                response = LLMChatResponse(
                    pred="", usage=total_usage, error=f"<agent_error>{traceback.format_exc()}</agent_error>",
                    context=self.evidence, question=question
                )
                break
        ...
        return LLMChatResponse(pred=response.pred, usage=total_usage, error=response.error, context=self.evidence, question=question)
...
def get_question_rewrite_from_sca_context_corpus(sca_context_corpus: pd.DataFrame, question_id: str, original_query: str) -> str:
    # logger.info(f"Getting query rewrite for question_id: {question_id}")
    # logger.info(f"Original query: {original_query}")
    query_rewrite_series = sca_context_corpus[sca_context_corpus["id"] == question_id]["query_rewrite"]
    if query_rewrite_series.empty:
        logger.warning(f"No query_rewrite found in sca_context_corpus for question_id: {question_id}")
        raise ValueError(f"No query_rewrite found in sca_context_corpus for question_id: {question_id}")
    else:
        # .iloc[0] extracts the first (and only) item from the Series as a string.
        query_rewrite = query_rewrite_series.iloc[0]
        # logger.info(f"Query rewrite: {query_rewrite}")
        return original_query if query_rewrite == "" else query_rewrite
