...
"""
In the dataset, the names are unique. Use name as identifer for each evidence.
"""
class RerankerLLMPrompt(LLMPrompt):
    ...
    Remember the instructions: You are an expert LLM evaluator that excels at evaluating a QUESTION and REFERENCES depending on how relevant and sufficient the references are to answering the question. Please rate the following QUESTION and REFERENCES to output the rankings from most relevant to least relevant. Include each title exactly once in the output list.
    ### QUESTION
    {{question}}
    ### REFERENCES
    {{evidence}}
    ...
    def get_prompt(self, prolog_query: bool = False) -> PromptTemplate:
      ...
      Args:
          prolog_query: This parameter is not used for LLM Reranker prompts, as they do not support Prolog query generation.

      Returns:
          A PromptTemplate object containing the LLM Reranker prompt template.
      """
      return PromptTemplate(
          input_variables=["example", "question", "evidence"],
          template=self.RERANKER_INSTRUCTION,
      )
...
Answer: Based on the evidence, the husband of Lannie Smock is Alvaro Smock. <answer>Alvaro Smock</answer>.

Example 5:
Question: Who is the person whose occupation is biomedical scientist?
Answer: Based on the evidence, the person whose occupation is biomedical scientist is Lannie Smock. <answer>Lannie Smock</answer>.

Example 6:
Question: Who is the brother of Aida Wang?
Answer: Based on the evidence, Aida Wang has no brothers. <answer></answer>.

Example 7:
Question: Who is the daughter of Alvaro Smock?
Answer: Based on the evidence, Alvaro Smock has no daughters. <answer></answer>.

Example 8:
Question: Who is the son of Ryan Wang?
Answer: Based on the evidence, Ryan Wang has no sons. <answer></answer>.

Example 9:
Question: Who is the person whose occupation is community education officer?
Answer: Based on the evidence, there is no person whose occupation is community education officer. <answer></answer>.
...
class CoTLLMPrompt(LLMPrompt):
    ...
    - numbers separated by '{constants.answer_sep}' (if the answer is numerical); or
    ...
    Here, FINAL_ANSWER must be the prolog query that will retrieve the answer to the question.
...
def get_llm_prompt(method: str, model_name: str) -> LLMPrompt:
    ...
    match method:
        case "zeroshot" | "zeroshot-sc" | "zeroshot-sca" | "zeroshot-reranker" | "zeroshot-rag-reranker":
            return ZeroshotLLMPrompt()
        case "fewshot" | "fewshot-sc" | "fewshot-rag" | "fewshot-sca" | "fewshot-reranker" | "fewshot-rag-reranker":
            return FewshotLLMPrompt()
        case "cot" | "cot-sc" | "cot-sc->react" | "cot-rag" | "cot-sca" | "cot-reranker" | "cot-rag-reranker":
            return CoTLLMPrompt()
        case "zeroshot-rag":
            return ZeroshotLLMPrompt()
